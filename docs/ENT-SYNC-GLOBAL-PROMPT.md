...
3. Side Restrictions & Alt Player Props: Alt player props are strictly derived from positive EV OVER candidates (edge_pct > 0) from the main board (both rejected and non-rejected), completely bypassing historical hit-rate requirements. All alternate player props MUST be exclusively available on Hard Rock. MLB alt game/team totals are OVER runs only and must be parlayed across different games. Home Runs (HR) remain pack-excluded entirely (desk hard ban via EXCLUDED_MARKETS).
