...
- Validate with time-based backtests using log loss, Brier score, calibration curves, CRPS/count error, and coverage by sport/market family. Promotion remains a manual configuration change after review of these reports, controlled by a checked-in YAML config keyed by sport and market family, with audit logging of who/when/version. The environment-variable override exists for local/non-prod testing only; it must emit the same who/when/version audit record as the YAML path so it cannot become a silent bypass of the promotion audit trail in production.
