...
2. **Files Touched**:
   - `outlier_scrapers/results.py`: (a) `collect_settlement_rows()` overwrote `candidates` with `_player_event_candidates()` for every selection containing `" - "`, so matchup-prefixed team totals were handed to the boxscore player lookup, matched nobody, and were discarded as unmatched before `_grade_row()` ran. The whole matchup team-total path from a326f62 was therefore dead in production since it landed, including the `IND @ TOR` case it was written for -- those bets never settled. `_grade_row()` already refused to grade such a selection as a player prop; that predicate is now `_is_matchup_prefixed()` and the collector applies the same one. (b) Added `_matchup_team_score()`: the grader tested the away side first with plain containment, and one team's code is regularly a substring of the other team's display name ("LA" inside "ATLANTADREAM"), so once reachable it would settle an Atlanta Dream total against Los Angeles' score. Now prefers an exact alias match and requires the containment fallback to resolve to exactly one side, skipping rather than guessing.
   ...
   - `tests/test_nfl_roster.py`, `tests/test_results.py`: 6 regression tests, including two end-to-end collector tests.
3. **Correction**: the first version of this entry and of the PR body claimed wrong W/L was reaching the settled ledger. Copilot's review showed the row was discarded before grading, so it could not. **No ledger re-grading is needed.** The live defect was that these rows never settled at all.
4. **Verification**:
   - 337 passed / 2 skipped across the NFL suites; 20 passed on the two changed test files.
   - Full offline suite 1,079 passed / 43 skipped; every remaining failure is `ModuleNotFoundError` for a third-party dependency. PyPI is blocked in the cloud sandbox (`x-deny-reason: host_not_allowed`), so `structlog`, `sqlalchemy`, `openai`, `anthropic`, `google-genai` and `dateutil` could not be installed; `outlier_scrapers` tests were run against a local logging shim outside the repo. Hosted CI is authoritative and is green.
...
5. **Next Steps**:
   ...
   - Open, not fixed (data/behavioural decisions, see PR body): matchup team totals for codes that are not substrings of the display name (`WSH`/Washington, `PHX`/Phoenix, `LV`, `NY`, `LA`, `GS`) resolve to neither side and are skipped -- closing this needs a display-name to code map; `--window` runs clobber the canonical `*_latest.json` with a partial slate; `matches_kickoff_window` silently returns True for an unrecognized token.
