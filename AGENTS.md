...
Cloud ents (Codex Linux sandboxes, remote VMs without PowerShell) are exempt
from the Windows-specific `report-sync.ps1` call. Instead, the cloud STEP 0
equivalent is:

```bash
git clone https://github.com/DaSilvaDub/outlier.git && cd outlier
git fetch origin && git reset --hard origin/master
# Verify all 5 upgrade markers are present:
grep -q 'player_id'              outlier_scrapers/pack.py
...
The house rule (never run reasoning unless explicitly asked) still applies by
policy even though the `block-reasoning.ps1` hook is unavailable in non-PowerShell
environments.

See `docs/CLOUD-SANDBOX-LIMITATIONS.md` for the full list of sandbox constraints.
