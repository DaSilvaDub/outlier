...
def learned_multiplier_promotion_signal(
    ...
    *,
    ...
    settled = [
        row
        for row in rows
        if _text(row.get("win_loss_push")).upper() in {"W", "L", "PUSH"}
    ]
    graded = [
        row for row in settled if _text(row.get("win_loss_push")).upper() in {"W", "L"}
    ]
    ...
    gates = {
        ...
        and abs(calibration_gap)
        <= float(resolved.get("max_absolute_calibration_gap") or 0.0),
        ...
        "minimum_clv_coverage": clv_coverage
        >= float(resolved.get("min_clv_coverage") or 0.0),
