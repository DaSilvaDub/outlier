"""Best-effort A/B/C/D/E research-desk runner.

This module coordinates the paid model runners and records their status. It
does not manufacture a local betting report; the canonical outlier-ai-desk
skill owns the pack-only synthesis fallback when paid reasoning is unavailable.
"""
...
def orchestrate_desk(
    pack_dir: Path, *, steps: Sequence[str] | None = None, force: bool = False
) -> int:
    load_environment()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--force", action="store_true", help="Force re-run selected phases.")
    parser.add_argument("--steps", default=",".join(PHASES), help="Comma-separated phase subset.")
    args = parser.parse_args(argv)
    steps = [step.strip() for step in args.steps.split(",") if step.strip()]
    return orchestrate_desk(paths.PROJECT_ROOT / "packs" / args.date, steps=steps, force=args.force)
