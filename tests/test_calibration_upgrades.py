...
from outlier_scrapers import paths, slate_quality, stake_calibration
...
def test_live_stake_calibration_matches_policy_source_column():
    ...
    # The live artifact is regenerated by the nightly fit, so its fingerprint and
    # sample count move on every refresh. Pin the identity contract instead of a
    # snapshot: the stamped version must be the v1 fingerprint of its own content.
    fingerprint = stake_calibration.compute_artifact_fingerprint(artifact)
