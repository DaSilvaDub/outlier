...
This is a **Master Card** report, produced for a human reader as Markdown.
...
* HR / home runs (any scope)
* HA / hits allowed, WALKS_ALLOWED (any scope)
* HRR / Hits + Runs + RBI (any scope)
* BB / walks **as a player prop** (a team walk total is not desk-prohibited by
  this rule; for MLB it is already outside the team whitelist below)
* Any MLB player prop that is not pitcher strikeouts (SO)
* Any MLB team prop that is not team runs (R) or team total (TOTAL)
