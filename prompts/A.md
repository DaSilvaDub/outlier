...
The Data block includes `candidates.csv` and, when present, `game_totals.csv` and/or `team_totals.csv` (deterministic projection boards; same schema, split streams). For game/team totals:
- Treat `edge_pct`, `fair_total`, `projected_over_prob`, and `actionable` in both CSVs as authoritative — do NOT recompute or alter them.
