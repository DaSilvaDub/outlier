...
5. **Active Roster & Quarterback Grounding Invariant:** Never infer or hallucinate NFL player-team affiliations (especially starting quarterbacks and skill players who changed teams, e.g. **Daniel Jones is on the Indianapolis Colts (IND)** (Anthony Richardson is NOT starting), **Aaron Rodgers is on the Pittsburgh Steelers (PIT)**, **Geno Smith is on the New York Jets (NYJ)**, **Carson Wentz is on the Minnesota Vikings (MIN)**, **Cam Ward is on the Tennessee Titans (TEN)**) from historical pre-training memory. Always query and anchor to `outlier_nfl.roster.get_starting_qb(team)` and the normalized feed's verified `team`, `player_name`, and `matchup` in `nfl_rosters_latest.json` or `nfl_props_*.json`.
